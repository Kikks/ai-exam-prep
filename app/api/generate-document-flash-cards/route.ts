...
import { calculateFlashCardsCredits } from "@/lib/tokens";
...
export async function POST(req: Request) {
	...
	try {
		...
		credits = await calculateFlashCardsCredits(document.content ?? "");
		...
		const { data: transactionData, error: transactionError } = await supabase
			...
			.insert({
				amount: credits,
				description: `Flashcards Generation from document - ${
		...
		await inngest.send({
			name: "document/generate-flash-cards",
			data: {
				documentId,
				userId,
				credits,
