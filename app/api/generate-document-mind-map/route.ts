...
import { calculateMindMapCredits } from "@/lib/tokens";
...
export async function POST(req: Request) {
	...
	try {
		...
		credits = await calculateMindMapCredits(document.content ?? "");
		...
		const { data: transactionData, error: transactionError } = await supabase
			...
			.insert({
				amount: credits,
				description: `Mind Map Generation from document - ${
		...
		await inngest.send({
			name: "document/generate-mind-map",
			data: {
				documentId,
				userId,
				credits,
